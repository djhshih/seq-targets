...
def process_extron(gene_name, alt_name, genome):
    ...
    ## ! Directly extracted:
    full_command = f'awk -F\'[\\t;]\' \'{gene_name_cmd} && $3==\"exon\" {{print $1,$4-1,$5-1}}\' {genome} >> extron.interval_list\n' 
    return(full_command)
...
def process_3utr(gene_name, alt_name, genome):
    gene_name_cmd = match_gene_name(gene_name, alt_name)
    ## ! Direct extract:
    full_command = f'awk -F\'[\\t;]\' \'{gene_name_cmd} && $3==\"three_prime_UTR\" {{print $1,$4-1,$5-1}}\' {genome} >> 3utr.interval_list\n' 
    return(full_command)
